...
from .models import db, Usuario, MusicTrack, Playlist, PlaylistTrack, PlaylistCollaborator
...
def list_tracks():
    ...
    tracks = MusicTrack.query.filter_by(user_id=current_user_id).order_by(MusicTrack.created_at.desc()).all()
...
def get_track(track_id):
    ...
    shared = db.session.query(PlaylistCollaborator).join(Playlist).join(PlaylistTrack).filter(
...
def confirm_upload(track_id):
    ...
    current_user_id = get_jwt_identity()
    track = MusicTrack.query.filter_by(id=track_id, user_id=current_user_id).first_or_404()
...
def list_playlists():
    ...
    shared = Playlist.query.join(PlaylistCollaborator).filter(
        PlaylistCollaborator.user_id == current_user_id
    ).all()
...
def get_playlist(playlist_id):
    ...
    tracks = db.session.query(MusicTrack).join(PlaylistTrack).filter(
...
def add_collaborator(playlist_id):
    ...
    current_user_id = get_jwt_identity()
    playlist = Playlist.query.filter_by(id=playlist_id, user_id=current_user_id).first_or_404()
