# src/api_v1_routes.py
# Rutas API v1 (prefijo /api/v1)
from flask import Blueprint, request, jsonify
from flask_jwt_extended import create_access_token, jwt_required, get_jwt_identity
from .models import Usuario, Mensaje
from . import db, r, limiter
...
# ---------- Mensajes ----------
@api_v1.route("/mensajes", methods=["GET"])
@jwt_required()
def api_mensajes():
    """GET mensajes - requiere autenticación"""
    mensajes = Mensaje.query.order_by(Mensaje.id.desc()).all()
    data = [{"id": m.id, "nombre": m.nombre, "mensaje": m.mensaje} for m in mensajes]
    return jsonify(data)


@api_v1.route("/mensajes", methods=["POST"])
@jwt_required()
def api_crear_mensaje():
    ...
    if estado not in ESTADOS_VALIDOS:
        return jsonify(
            {
                "error": f"Estado inválido. Valores permitidos: {', '.join(sorted(ESTADOS_VALIDOS))}"
            }
        ), 400
    ...
    if not nuevo.proyecto_id:
        return jsonify(
            {
                "error": "No se puede crear una tarea sin asignarla a un proyecto. Por favor selecciona un proyecto antes de continuar."
            }
        ), 400
    ...
    return jsonify(
        {
            ...
            "data": {
                ...
                "estado": nuevo.estado,
                "start_date": nuevo.start_date.isoformat()
                if nuevo.start_date
                else None,
                "expiration_date": nuevo.expiration_date.isoformat()
                if nuevo.expiration_date
                else None,
            },
            "autor": user_id,
        }
...
def login():
    """Login con rate limiting para prevenir brute force"""
    data = request.get_json()
...
def register():
    """Register con rate limiting para prevenir spam de cuentas"""
    data = request.get_json()
    username = data.get("username")
    password = data.get("password")
    ...
    nuevo_usuario = Usuario(
        username=username, password=ph.hash(password), rol="usuario"
    )
...
@api_v1.route("/mis-mensajes", methods=["GET"])
@jwt_required()
def mis_mensajes():
    user_id = get_jwt_identity()
    mensajes = (
        Mensaje.query.filter_by(usuario_id=user_id).order_by(Mensaje.id.desc()).all()
    )
    return jsonify(
        [
            {
                ...
                "expiration_date": m.expiration_date.astimezone(
                    timezone.utc
                ).isoformat()
...
@api_v1.route("/mensajes/<int:id>", methods=["PUT"])
@jwt_required()
        ...
        return jsonify(
            {
                ...
                "user_id": user_id,
                "mensaje_id": mensaje.usuario_id if mensaje else None,
            }
        ), 403
...
@api_v1.route("/mensajes/<int:id>", methods=["DELETE"])
@jwt_required()
...
@api_v1.route("/mis-mensajes/<int:id>/duplicate", methods=["POST"])
@jwt_required()
def duplicar_mensaje(id):
    ...
    nuevo = Mensaje(
        ...
        expiration_date=mensaje.expiration_date.astimezone(timezone.utc).isoformat()
        if mensaje.expiration_date
        else None,
    )
    db.session.add(nuevo)
    db.session.commit()
